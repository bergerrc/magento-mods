...
class RetornoPagSeguro
{
	...
	static function _preparaDados($post, $confirmacao=true)
	{
		...
		if ($confirmacao) {
	        $standard = Mage::getSingleton('pagseguro/standard');
			$token = $standard->getConfigData('token');
			if ( $post['Referencia'] ){
		        $order = Mage::getSingleton('sales/order');
		        $order = $order->loadByIncrementId($post['Referencia']);
				$token = $standard->getConfigData('token', $order->getStoreId());
			}
	...
	static function _tipoEnvio()
	{
		...
		if (function_exists('curl_exec')) {
			return array(
				'curl', 
				RetornoPagSeguro::_getStatusUrl()
				);
	...
	static function _getStatusUrl(){
        $standard = Mage::getSingleton('pagseguro/standard');
		return $standard->getPagSeguroStatusUrl();
	}
