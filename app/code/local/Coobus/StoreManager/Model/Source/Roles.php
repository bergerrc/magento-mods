 ...
 */
class Coobus_StoreManager_Model_Source_Roles
{
	
    public function toOptionArray()
    {
    	$acl = Mage::getResourceModel('admin/acl')->loadAcl();
    	$roles = Mage::getModel('admin/role')->getCollection()->setRolesFilter();
    	
    	$result = array(''=>'');
    	foreach ($roles as $role) {
    	    try {
                $data['value'] = $role->getId();
                $data['label'] = $role->getRoleName();
                $result[] = $data;
            } catch (Exception $e) {}
    	}
    	return $result;
