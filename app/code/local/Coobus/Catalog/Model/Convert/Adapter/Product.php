    ...
    extends Mage_Catalog_Model_Convert_Adapter_Product
{
	...
	public function load(){
	    if ($filter_ids = Mage::registry('catalog_product_filter_ids')){

            $attr = array();
            $attr['alias']       = 'prod2';
            $attr['attribute']   = 'catalog/product';
            $attr['field']       = 'qty';
            $attr['bind']        = 'entity_id=entity_id';
            $attr['cond']        = "{{table}}.entity_id in (".implode(',',$filter_ids).")";
            $attr['joinType']    = 'inner';

            $this->setJoinField($attr);
        }
		parent::load();
	}
    ...
    public function saveRow(array $importData)
    {
        ...
        foreach ($inventoryFields as $field) {
            if (isset($importData[$field])) {
                ...
                if ( $field == 'backorders' && !is_numeric($importData[$field]) ){
			        $options = array(
				        Mage::helper('adminhtml')->__('No')=>Mage_CatalogInventory_Model_Stock::BACKORDERS_NO,
				        Mage::helper('adminhtml')->__('Yes')=>Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NOTIFY,
				        Mage::helper('adminhtml')->__('Specified')=>Mage_CatalogInventory_Model_Stock::BACKORDERS_YES_NONOTIFY
			        );
                	$stockData[$field] = $options[ $importData[$field] ];               	
                }
            }
        }
    ...
    protected function _addCategories($categories, $store)
    {
        ...
        foreach (explode(',', $categories) as $categoryPathStr) {
            ...
            //$path = $rootPath;
            $namePath = strtolower(reset($cache)->getName());
