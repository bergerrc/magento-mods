 ...
 */
class Coobus_Catalog_Block_Widget_Grid_Column_Renderer_Image extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    public function __construct() {
        if(self::$showImagesUrl == null)
            self::$showImagesUrl = (int)Mage::getStoreConfig('catalog/management/showurl') === 1;
        if(self::$showByDefault == null)
            self::$showByDefault = (int)Mage::getStoreConfig('catalog/management/showbydefault') === 1;
        if(self::$width == null)
            self::$width = Mage::getStoreConfig('catalog/management/width');
        if(self::$height == null)
            self::$height = Mage::getStoreConfig('catalog/management/height');
    }
    ...
    protected function _getValue(Varien_Object $row)
    {
        ...
        $url = Mage::helper('coobus_catalog')->getImageUrl($val);
        
        if(!Mage::helper('coobus_catalog')->getFileExists($val)) {
          $dored =true;
          $val .= "[!]";
        }
