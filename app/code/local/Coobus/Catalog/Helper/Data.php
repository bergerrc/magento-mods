...
class Coobus_Catalog_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getSearchCollection($queryString, $request) {
    	...
    	if(Mage::helper('coobus_catalog')->isMageVerAtLeast('1.4.0.0')) {
    ...
    public function filter( $text ){
    	$filter = Mage::getModel('coobus_skin/template_filter');
    	return $filter->filter($text);
    }    
    
    public function getShowPager(){
    	if ( Mage::app()->getRequest()->getActionName() == 'tab' )
    		$tabname = Mage::app()->getRequest()->getParam('active', false) . 'products';
    	else
	    	$tabname = 'allproducts';
    	return (int)Mage::getStoreConfig("catalog/". $tabname . "/show_pager");
    }
}
?>