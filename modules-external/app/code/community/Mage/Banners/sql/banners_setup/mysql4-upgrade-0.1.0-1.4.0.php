...
$installer->run("
DROP TABLE IF EXISTS `{$this->getTable('banners_website')}`;
CREATE TABLE IF NOT EXISTS `{$this->getTable('banners_website')}` (
`banners_id` int(11) NOT NULL,
`website_id` smallint(5) unsigned NOT NULL,
		 PRIMARY KEY  (`banners_id`,`website_id`),                      
		 KEY `FK_BANNERS_WEBSITE_WEBSITE` (`website_id`)                    
		) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Banners Websites';

");
$connection->addConstraint('FK_BANNERS_WEBSITE_WEBSITE',
    $installer->getTable('banners_website'), 'website_id',
    $installer->getTable('core_website'), 'website_id',
    'CASCADE', 'CASCADE', true
);

$connection->addConstraint('banners_website_ibfk_1',
    $installer->getTable('banners_website'), 'banners_id',
    $installer->getTable('banners'), 'banners_id',
    'CASCADE', 'CASCADE', true
);
$installer->endSetup(); 
