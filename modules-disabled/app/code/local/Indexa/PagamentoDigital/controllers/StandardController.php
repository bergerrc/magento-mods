...
class Indexa_PagamentoDigital_StandardController extends Mage_Core_Controller_Front_Action
{
    ...
    public function redirectAction()
    {
        ...
        if ( $session->getStoreId() )
        	$session->setPagamentodigitalStandardStoreId( $session->getStoreId() );
    ...
    public function  successAction()
    {
		...
		if ( trim($dados_post_status) == "Transação em Andamento" || $dados_post_status == "Transa��o em Andamento")
		{
        ...
        {
        	$tmp_order = Mage::getModel('sales/order');
        	$tmp_order->loadByIncrementId($dados_post['id_pedido']);
        	$storeId = $tmp_order->getStoreId();

			$token = $this->getStandard()->getConfigData('token',$storeId);
			...
			if(trim($resposta) == "VERIFICADO")
			{
	        	...
	        	else
	        	{
	                ...
	                {
				        if ( $dados_post_status == "Transação Concluída" || $dados_post_status == "Transa��o Conclu�da" )
				        {
						...
						elseif ( trim($dados_post_status) == "Transação Cancelada" || $dados_post_status == "Transa��o Cancelada" )
						{
			...
			else
			{
	        	...
	        	else
		        {
					...
					$order->addStatusToHistory(
						$order->getStatus(),
						Mage::helper('pagamentodigital')->__('Pagamento Digital enviou automaticamente uma verificação inválida: %s', $comment . ' Resposta do PD: '. $resposta));
					$order->save();
		        }
			}
        }
